...
import { CopyConditions, Client as MinioClient, BucketItem, IncompleteUploadedBucketItem } from "minio";
import { Errors, type Service, type ServiceSchema, ServiceSettingSchema } from "moleculer";
...
export interface SettingsSchema {
  ...
  transport?: string;
  sessionToken?: string;
  minioHealthCheckInterval?: number;
  pathStyle?: boolean;
}

interface ServiceThis extends Service<SettingsSchema> {
  client: MinioClient;
}

const MinioMixin: ServiceSchema<ServiceSettingSchema | SettingsSchema> = {
  // Service name
  name: "minio",
  ...
  methods: {
     ...
     */
    ping(this: ServiceThis, { timeout = 5000 } = {}) {
      return Promise.race([
  ...
  actions: {
    ...
    makeBucket: {
      ...
      handler(this: ServiceThis, ctx) {
        return Promise.resolve(ctx.params).then(
          ({ bucketName, region = "" }) =>
            this.client.makeBucket(bucketName, region, {
              ObjectLocking: !!ctx.meta.lock,
            }),
        );
      },
    },
    ...
    listObjects: {
      ...
      handler(this: ServiceThis, ctx) {
        return Promise.resolve(ctx.params).then(
          ({ bucketName, prefix = "", recursive = false }) => new Promise((resolve, reject) => {
            try {
    ...
    listObjectsV2: {
      ...
      handler(this: ServiceThis, ctx) {
        return Promise.resolve(ctx.params).then(
          ({ bucketName, prefix = "", recursive = false, startAfter = "" }) => new Promise((resolve, reject) => {
            try {
        ...
        return Promise.resolve(ctx.params).then(
          ({ bucketName, prefix = "", recursive = false }) => new Promise((resolve, reject) => {
            try {
              ...
              const objects: IncompleteUploadedBucketItem[] = [];
              stream.on("data", (el) => objects.push(el));
              stream.on("end", () => resolve(objects));
              stream.on("error", reject);
            } catch (e) {
    ...
    presignedPutObject: {
      ...
      handler(this: ServiceThis, ctx) {
        return Promise.resolve(ctx.params).then(
          ({ bucketName, objectName, expires }) => new Promise((resolve, reject) => {
    ...
    presignedPostPolicy: {
      ...
      handler(this: ServiceThis, ctx) {
        return Promise.resolve(ctx.params).then(({ policy }) => {
